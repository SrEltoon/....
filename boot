...
interval: 15,
offset: 0,
msg: "Eu sou um programador e nao sou especial por isso huehue!"
}
];
...
pupOnline = function() {
return API.sendChat("Programa LIGADO!");
};
...
user.warn();
_results.push(API.sendChat("@" + user.getUser().username + ",Eu não vi você conversar ou votar em pelo menos 12 minutos. Se você não mostrar  atividade em 2 minutos, eu vou removê-lo."));
...
warnMsg += ",Eu não vi você conversar ou votar em pelo menos 14 minutos agora. Este é o seu segundo e último aviso. Se você não conversar ou votar no minuto seguinte, vou removê-lo.";
...
API.sendChat("@" + user.getUser().username + ", Você já tinha duas advertências. Por favor, fique mais ativo.");
...
var msg, nameIndex;
msg = "Não gostamos de djs AUSENTES. Nós calculamos o seu estado AUSENTE marcando a última vez que você votou ou falou. Se você não votar ou falar, eu vou automaticamente removê-lo. Utilize o nosso roteiro AutoWoot para evitar ser removido: http://bit.ly/McZdWw";
...
var msg;
msg = "Toque tudo, menos funk. ";
msg += "Mantenha a música relacionada com o tema da sala, ou vamos ter que salta-la.";
return API.sendChat(msg);
};
return themeCommand;
})(Command);
...
var msg;
msg = "1) Toque musicas de qualidade. ";
msg += "2) Não repita as musicas, ou elas seram puladas. ";
msg += "3) Musicas até 6min e 30seg sao permitidas. ";
msg += "4) Tente nao ficar ausente. ";
msg += "5) Respeite Admins e Mods. ";
return API.sendChat(msg);
};
return rulesCommand;
})(Command);
...
var msg1, msg2;
msg1 = "Crie uma lista de reprodução e preencha com músicas do YouTube ou Soundcloud.";
msg1 += "Clique no botão 'Clique para DJ' e espere a sua vez de tocar música. Para ver quais gêneros musicais são permitidos, digite '/theme'.";
msg2 = "Tocar música de boa qualidade que não foi tocada recentemente (confira a história do quarto). ";
msg2 += "Pergunte a um mod se você não tiver certeza sobre sua escolha da música..";
API.sendChat(msg1);
return setTimeout((function() { 
...
var msg;
msg = 'Um carinha criou, eu fui lá peguei e editei melhor ainda!';
return API.sendChat(msg);
};
...
var msg, nameIndex;
msg = "Por favor dê BACANA na cabine de DJ ou será removido dela!";
if ((nameIndex = this.msgData.message.indexOf('@')) !== -1) {
return API.sendChat(this.msgData.message.substr(nameIndex) + ', ' + msg);
} else {
return API.sendChat(msg);
...
var msg;
msg = "Puta merda, que musica ruim. Porra Acreano, tira isso!";
return API.sendChat(msg);
};
...
if (msg === '') {
return API.sendChat("Ninguém está ausente no momento");
} else {
...
if (msg === '') {
return API.sendChat("Ninguém está ausente");
} else {
return API.sendChat('AFKs: ' + msg);
}
};
...
totals = '' + t.songs + ' Músicas tocadas, acumularam ' + t.woots + ' woots, ' + t.mehs + ' mehs, and ' + t.curates + ' queues.';
msg = launch + totals;
return API.sendChat(msg);
};
...
API.sendChat('Naaaao, nao me mateee...');
undoHooks();
API.sendChat('Pooor favooor, eu suplicooo...');
data.implode();
return API.sendChat('Aii que morte deliciosa x_x');
};
return dieCommand;
})(Command);
...
msg = this.msgs[m].replace('{fb}', 'https:/www.facebook.com/eltoon17');
return API.sendChat(msg);
};
fbCommand.prototype.msgs = ["Se você tiver quaisquer dúvidas, sugestões ou simplesmente quer conversar. Me mande mensagem no facebook!: {fb}"];
return fbCommand;
})(Command);
...
return API.sendChat("Essa música é tão ruim! Melhor fazer uma nova lista!");
};
...
var msg;
msg = "Reserve Mehs para músicas que são 1) Extremamente Exagerado 2) Fora do Gênero 3) Absolutamente Horrível ou 4) Músicas Trolls. ";
...
return API.sendChat("/em: " + obj.user.username + " Gostou e adicionou sua musica!");
};
handleUserJoin = function(user) {
data.userJoin(user);
data.users[user.id].updateActivity();
return API.sendChat("/em: " + user.username + ", Juntou-se a sala!");
};
...
API.sendChat("/em: Foi tocada " + data.currentsong.title + " by " + data.currentsong.author + ". Stats: :+1: " + data.currentwoots + ", :-1: " + data.currentmehs + ", :heart: " + data.currentcurates + ".");
data.newSong();
document.getElementById("button-vote-positive").click();
}
if (data.forceSkip) {
...
API.sendChat("Não faça spam links de salas, por favor.");
return API.moderateDeleteChat(chat.chatID);
} else {
return API.sendChat("Eu deveria expulsá-lo, mas estamos aqui para nos divertir.");
}
}
}
};
...
responses = ["@{beggar}Ganhe seus fãs tocando musica boa, nao mendigando!", "Cara @{beggar} para de perguntar por fãs. ", "@{beggar}Se ele(a) gostar de ti, ele vai te dá fã ", "@{beggar}. Ganhando seus fãs a boa moda antiga. Tocando musicas boas e tendo boas conversas."];
